...
export default function SpecifyDocumentSignatureInViewer({
  handleSave,
  document,
  handleClose,
  isOpen,
  ...
  handleSave: (signedBy: Array<string>) => Promise<boolean>;
}) {
  ...
  const upload = async () => {
    setLoading(true);
    const response = await handleSave(selectedMembers);
    setLoading(false);
    if (response) {
      closeModal();
    }
          ...
          <div className="flex w-full flex-col">
            <div className="flex flex-row justify-between px-2">
              ...
              <div className="flex flex-row items-center">
                <span>Todos:</span>
                <Spacer />
                <Checkbox isSelected={selectedAll} onChange={toggleSelectAll} />
              </div>
            </div>
              ...
              <div className="flex flex-col p-2" key={index}>
                <div className="flex flex-row justify-between pb-2">
                  ...
                  <Checkbox
                    ...
                    onChange={() => toggleSelectMember(signer.signerPosition!)}
                  />
                </div>
